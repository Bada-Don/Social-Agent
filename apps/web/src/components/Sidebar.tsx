...
import { LayoutDashboard, Inbox, CheckSquare, BrainCircuit, Contact, Settings } from "lucide-react";
  ...
  return (
    <aside className="w-64 h-screen border-r border-white/10 bg-surface p-4 flex flex-col gap-2">
      <div className="mb-8 px-2 flex items-center gap-3">
        ...
        <span className="font-semibold tracking-wide text-gray-200">Executive OS</span>
      </div>
      
      <nav className="flex-1 space-y-1">
        {navItems.map((item) => (
          <Link key={item.name} href={item.href} className="flex items-center gap-3 px-3 py-2 rounded-md text-gray-400 hover:text-white hover:bg-white/5 transition-all">
            <item.icon size={18} />
